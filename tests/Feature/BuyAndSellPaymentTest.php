...
class BuyAndSellPaymentTest extends TestCase
{
    ...
    public function it_creates_and_updates_property_buy_with_partial_payment_and_auto_due_calculation()
    {
        ...
        $this->assertEquals(3000000.0, (float) $pm->purchase_price);
        $this->assertEquals(1000000.0, (float) $pm->paid_amount);
        $this->assertEquals(2000000.0, (float) $pm->due_amount);
        ...
        $pm->refresh();
        $this->assertEquals(3000000.0, (float) $pm->paid_amount);
        $this->assertEquals(0.0, (float) $pm->due_amount);
    ...
    public function it_creates_and_updates_property_sale_with_partial_payment_and_auto_due_calculation()
    {
        $property = Property::firstOrCreate(
            ...
            ['firm_id' => $this->firm->id, 'property_name' => 'Sell Test Plot #88', 'price' => 4500000.0, 'status' => 'available']
        );
        ...
        $this->assertEquals(4500000.0, (float) $sale->sale_amount);
        $this->assertEquals(1500000.0, (float) $sale->booking_amount);
        $this->assertEquals(3000000.0, (float) $sale->remaining_amount);
        ...
        $sale->refresh();
        $this->assertEquals(4500000.0, (float) $sale->booking_amount);
