...
class StockInwardController extends Controller
{
    ...
    public function index(Request $request)
    {
        $user = Auth::user();
        $isAdmin = $user && $user->isAdmin();
        $query = StockInward::with(['material.materialCategory', 'project.propertyMaster', 'property', 'purchaseOrder', 'contractor']);
        ...
        return view('admin.stock-inwards.index', compact('inwards', 'materials', 'projects', 'contractors'));
    }
    ...
    public function getPendingItems(PurchaseOrder $purchaseOrder)
    {
        ...
        return response()->json([
            ...
            'contractor_id'   => $purchaseOrder->contractor_id,
            'contractor_name' => $purchaseOrder->contractor->contractor_name ?? '',
    ...
    public function print(StockInward $stockInward)
    {
        ...
        if ($stockInward->inward_number) {
            $inwards = StockInward::where('inward_number', $stockInward->inward_number)->with(['material.materialCategory', 'project.propertyMaster', 'contractor'])->get();
            $inwardGroup = $inwards->first();
            return view('admin.stock-inwards.show', compact('inwardGroup', 'inwards'))->with('printMode', true);
        } else {
            $stockInward->load(['material.materialCategory', 'project.propertyMaster', 'contractor']);
            ...
            try {
                foreach ($request->items as $itemData) {
                    ...
                    $inward = StockInward::create([
                        ...
                        'contractor_id'     => $request->contractor_id ?: ($po->contractor_id ?: null),
    ...
    public function show(StockInward $stockInward)
    {
        ...
        if ($stockInward->inward_number) {
            $inwards = StockInward::where('inward_number', $stockInward->inward_number)->with(['material.materialCategory', 'project.propertyMaster', 'property', 'contractor'])->get();
            $inwardGroup = $inwards->first();
            return view('admin.stock-inwards.show', compact('inwardGroup', 'inwards'));
        } else {
            $stockInward->load(['material.materialCategory', 'project.propertyMaster', 'property', 'contractor']);
