...
<div class="crud-header">
    <div class="crud-title">
        <h2>Edit Expense</h2>
        <p>Update expense voucher — <strong>{{ $expense->expense_title ?? $expense->expense_category }}</strong></p>
    </div>
</div>
...
<div class="card-box">
    <form method="POST" action="{{ route('expenses.update', $expense->id) }}" enctype="multipart/form-data" id="expense-form">
        ...
        @php
            $currType = old('expense_type', $selectedType ?? ($expense->expense_type ?: 'Property'));
        @endphp
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                ...
                @foreach($rentalChips as $chip)
                    <button type="button" class="rental-chip-btn"
                            ...
                            style="background: {{ old('expense_subcategory', $expense->expense_subcategory) == $chip['cat'] ? '#2563EB' : 'rgba(30, 41, 59, 0.70)' }}; border: 1px solid rgba(255, 255, 255, 0.12); color: #E2E8F0; padding: 6px 12px; border-radius: 20px; font-size: 12.5px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; transition: all .2s ease;">
                        <i class="fa-solid {{ $chip['icon'] }}" style="color: #60A5FA;"></i>
                        <span>{{ $chip['cat'] }}</span>
                    </button>
                @endforeach
            </div>
            <input type="hidden" name="expense_subcategory" id="expense_subcategory" value="{{ old('expense_subcategory', $expense->expense_subcategory) }}">
        </div>
        ...
        <div class="form-section" id="section-classification">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        @php
                            $currentCat = old('expense_category', $expense->expense_category ?: ($expense->expenseCategory->name ?? ''));
                        @endphp
                        <select name="expense_category" id="expense_category" class="form-control @error('expense_category') is-invalid @enderror" onchange="handleCategoryDropdownChange(this)" required>
                            ...
                            @if(isset($categories))
                                ...
                                @endforeach
                            @endif
                            @if(!empty($currentCat) && (empty($categories) || !$categories->pluck('name')->contains($currentCat)))
            ...
            <div class="form-row" id="project-property-row" style="{{ in_array($currType, ['General', 'Personal']) ? 'display:none;' : '' }}">
                ...
                <div class="form-group" id="property-field-wrapper" style="{{ $currType === 'Rental' ? 'grid-column: 1 / -1;' : '' }}">
                    ...
                    @php
                        $existingPropIds = $expense->relationLoaded('properties') && $expense->properties->isNotEmpty()
                            ? $expense->properties->pluck('id')->toArray()
                            : ($expense->property_id ? [$expense->property_id] : []);
                        $selectedPropIds = (array) old('property_ids', $existingPropIds);
                    @endphp
            ...
            <div id="rental-connection-card" style="{{ $currType === 'Rental' ? '' : 'display:none;' }} margin-top: 20px; background: rgba(15, 23, 42, 0.85); border: 1.5px solid rgba(59, 130, 246, 0.35); border-radius: 18px; padding: 22px; box-shadow: 0 10px 30px rgba(0,0,0,0.35);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 12px;">
                    ...
                    <div id="rental-sync-badge" style="display: inline-flex; align-items: center; gap: 6px; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; background: rgba(59, 130, 246, 0.15); border: 1px solid rgba(59, 130, 246, 0.4); color: #93C5FD;">
                        ...
                        <span id="rental-status-text">{{ $expense->rental ? '✅ Linked to Agreement ' . ($expense->rental->agreement_no ?: '#'.$expense->rental->id) : 'Select a Property to Auto-Fetch Details' }}</span>
                    </div>
                </div>
                            ...
                            @if(isset($rentals))
                                @foreach($rentals as $r)
                                    <option value="{{ $r->id }}"
                                            ...
                                            {{ old('rental_id', $selectedRentalId ?? $expense->rental_id) == $r->id ? 'selected' : '' }}>
                            ...
                            @if(isset($tenants))
                                @foreach($tenants as $t)
                                    <option value="{{ $t->id }}"
                                            ...
                                            {{ old('tenant_id', $selectedTenantId ?? $expense->tenant_id) == $t->id ? 'selected' : '' }}>
                ...
                @php
                    $linkedRental = $expense->rental;
                    $linkedTenant = $expense->tenant ?? $linkedRental?->tenant;
                @endphp
                <div id="rental-snapshot-box" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; background: rgba(0,0,0,0.30); border-radius: 12px; padding: 14px; border: 1px solid rgba(255,255,255,0.06);">
                    ...
                    <div>
                        ...
                        <div id="snap-rent-amt" style="font-size: 14px; font-weight: 800; color: #F59E0B; margin-top: 2px;">{{ $linkedRental ? '₹ '.number_format($linkedRental->rent_amount, 2).' / mo' : '₹ 0.00 / mo' }}</div>
                        <div id="snap-deposit-amt" style="font-size: 11.5px; color: #94A3B8;">Dep: {{ $linkedRental ? '₹ '.number_format($linkedRental->security_deposit, 2) : '₹ 0.00' }}</div>
                    </div>
                    <div>
                        <div style="font-size: 11px; text-transform: uppercase; color: #94A3B8; font-weight: 700; letter-spacing: 0.5px;">Lease Period</div>
                        <div id="snap-lease-period" style="font-size: 12.5px; font-weight: 600; color: #CBD5E1; margin-top: 3px;">
                            @if($linkedRental && $linkedRental->start_date)
                                {{ is_string($linkedRental->start_date) ? $linkedRental->start_date : $linkedRental->start_date->format('d M Y') }}
                                @if($linkedRental->end_date)
                                    → {{ is_string($linkedRental->end_date) ? $linkedRental->end_date : $linkedRental->end_date->format('d M Y') }}
                                @endif
                            @else
                                —
                            @endif
                ...
                <div style="margin-top: 18px; padding-top: 16px; border-top: 1px dashed rgba(255,255,255,0.12);">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; margin: 0;">
                            <input type="checkbox" name="is_tenant_recoverable" id="is_tenant_recoverable" value="1"
                                   {{ old('is_tenant_recoverable', $expense->is_tenant_recoverable) ? 'checked' : '' }}
                    ...
                    <div id="recovery-fields-row" class="form-row" style="{{ old('is_tenant_recoverable', $expense->is_tenant_recoverable) ? '' : 'display:none;' }} margin-top: 14px;">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" name="recovery_amount" id="recovery_amount"
                                   value="{{ old('recovery_amount', $expense->recovery_amount) }}" class="form-control"
                        ...
                        <div class="form-group">
                            ...
                            <select name="recovery_status" id="recovery_status" class="form-control">
                                @foreach(['Pending', 'Recovered', 'Partially Recovered', 'Waived', 'Deducted from Deposit'] as $st)
                                    <option value="{{ $st }}" {{ old('recovery_status', $expense->recovery_status ?? 'Pending') == $st ? 'selected' : '' }}>{{ $st }}</option>
                                @endforeach
                            </select>
            ...
            <div class="form-row-3">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" id="amount"
                           value="{{ old('amount', $expense->amount) }}" class="form-control @error('amount') is-invalid @enderror"
                ...
                <div class="form-group">
                    ...
                    <select name="payment_mode" id="payment_mode" class="form-control @error('payment_mode') is-invalid @enderror">
                        ...
                        @foreach($paymentModes as $pm)
                            ...
                            <option value="{{ $pName }}" {{ old('payment_mode', $expense->payment_mode ?? 'Cash') == $pName ? 'selected' : '' }}>{{ $pName }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 8px;">
                        <input type="text" name="paid_to" id="paid_to" value="{{ old('paid_to', $expense->paid_to ?? ($expense->vendor->name ?? '')) }}"
                        ...
                        <input type="hidden" name="vendor_id" id="vendor_id" value="{{ old('vendor_id', $expense->vendor_id) }}">
                    </div>
            ...
            <div class="form-row-3">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="payment_account" id="payment_account"
                           value="{{ old('payment_account', $expense->payment_account) }}"
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select name="approval_status" id="approval_status" class="form-control @error('approval_status') is-invalid @enderror">
                        @foreach(['Pending','Approved','Rejected'] as $s)
                            <option value="{{ $s }}" {{ old('approval_status', $expense->approval_status ?? 'Pending') == $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="expense_title" id="expense_title"
                           value="{{ old('expense_title', $expense->expense_title) }}" class="form-control @error('expense_title') is-invalid @enderror"
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
                              placeholder="Enter details about what this expense was incurred for...">{{ old('description', $expense->description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror"
                              placeholder="Add any additional notes, remarks or audit references...">{{ old('notes', $expense->notes ?? $expense->remarks) }}</textarea>
            ...
            <div class="form-group">
                @if($expense->bill_file)
                    <div class="current-file">
                        ...
                        <div>
                            <div style="font-size:12px;color:#94A3B8;margin-bottom:2px;">Current attached file:</div>
                            <a href="{{ asset('storage/'.$expense->bill_file) }}" target="_blank">
                                <i class="fa-solid fa-arrow-up-right-from-square" style="font-size:11px;"></i>
                                View Current Attachment ({{ basename($expense->bill_file) }})
                            </a>
                        </div>
                    </div>
                    <div style="font-size:12px;color:#CBD5E1;margin:8px 0 10px;">Upload a new file below only if you wish to replace the existing one.</div>
                @endif
                <div class="file-upload-box" onclick="document.getElementById('bill_file').click()">
                    <label class="file-upload-label" for="bill_file">
                        ...
                        <span class="upload-text">{{ $expense->bill_file ? 'Click to replace bill / receipt file' : 'Click to upload bill, invoice or receipt' }}</span>
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            if (rentalSelect && data.rentals && data.rentals.length > 0) {
                const curVal = rentalSelect.value;
                ...
                if (curVal && Array.from(rentalSelect.options).some(o => o.value == curVal)) {
                    rentalSelect.value = curVal;
                } else if (data.active_rental) {
                    rentalSelect.value = data.active_rental.id;
                }
            }

            // Populate/select tenant
            if (data.active_rental && data.active_rental.tenant_id && tenantSelect && !tenantSelect.value) {
