...
<div class="crud-header">
    <div class="crud-title">
        ...
        <p>Update property details, pricing, units &amp; vendor records.</p>
    </div>
...
<div class="form-card">
    <form method="POST" action="{{ route('property-masters.update', $propertyMaster->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <select name="property_type" id="property_type_select" class="form-control" onchange="handlePropertyTypeChange(this.value)">
                    ...
                    @foreach($propertyTypes as $key => $label)
                        <option value="{{ $key }}" {{ old('property_type', $propertyMaster->property_type) === $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- ================================================================
                 SECTION 2: PROPERTY SIZE, AREA & RATE
            ================================================================ -->
            ...
            <div class="form-group full-width" style="background: rgba(15, 23, 42, 0.65); border: 1.5px solid rgba(245, 158, 11, 0.30); border-radius: 14px; padding: 18px; margin-top: 4px; margin-bottom: 10px;">
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 16px;">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <input type="text" name="unit_numbers_list" id="unit_numbers_list" value="{{ old('unit_numbers_list', $propertyMaster->unit_numbers_list) }}" class="form-control" placeholder="e.g. 1-10, 30, 35 or 1 to 10, 30, 35" style="border-color: rgba(245, 158, 11, 0.4);" oninput="handleUnitNumbersInput()">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <input type="number" min="0" name="total_units_count" id="total_units_count" value="{{ old('total_units_count', $propertyMaster->total_units_count ?: $propertyMaster->plots->count()) }}" class="form-control" placeholder="e.g. 12" style="font-weight: 800; color: #F59E0B; border-color: rgba(245, 158, 11, 0.4);">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <input type="text" name="unit_prefix" id="unit_prefix" value="{{ old('unit_prefix', $propertyMaster->unit_prefix ?? 'Plot ') }}" class="form-control" placeholder="e.g. Plot, Flat, Shop">
                ...
                <div id="unit_preview_container" style="display: none; margin-top: 14px; padding-top: 12px; border-top: 1px dashed rgba(245, 158, 11, 0.25);">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px;">
                        ...
                        <label style="display: flex; align-items: center; gap: 8px; font-size: 12.5px; color: #34D399; font-weight: 700; cursor: pointer; background: rgba(16, 185, 129, 0.12); border: 1px solid rgba(16, 185, 129, 0.35); padding: 5px 12px; border-radius: 8px;">
                            <input type="checkbox" name="auto_generate_units" id="auto_generate_units" value="1" style="accent-color: #10B981; width: 16px; height: 16px; cursor: pointer;">
                            <span id="autoGenText"><i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing sub-plots/units on save</span>
                        </label>
                    </div>
                </div>
            </div>
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <div class="input-luxury-group border-emerald">
                            ...
                            <input type="number" step="0.01" name="purchase_price" id="purchase_price"
                                   ...
                                   placeholder="0.00" oninput="recalculatePayment()">
                        </div>
                        ...
                        <select name="seller_id" id="seller_id" class="form-control">
                            ...
                            @if(isset($sellers))
                                @foreach($sellers as $s)
                                    <option value="{{ $s->id }}" {{ old('seller_id', $propertyMaster->seller_id) == $s->id ? 'selected' : '' }}>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-top: 14px;">
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <select name="payment_mode" id="payment_mode" class="form-control">
                            <option value="Cash" {{ old('payment_mode', $propertyMaster->payment_mode ?? 'Cash') === 'Cash' ? 'selected' : '' }}>💵 Cash</option>
                            <option value="Cheque" {{ old('payment_mode', $propertyMaster->payment_mode) === 'Cheque' ? 'selected' : '' }}>📝 Cheque / Check</option>
                            <option value="Bank Transfer / RTGS / NEFT" {{ old('payment_mode', $propertyMaster->payment_mode) === 'Bank Transfer / RTGS / NEFT' ? 'selected' : '' }}>🏦 Bank Transfer / RTGS / NEFT</option>
                            <option value="UPI" {{ old('payment_mode', $propertyMaster->payment_mode) === 'UPI' ? 'selected' : '' }}>📱 UPI (GPay/PhonePe)</option>
                            <option value="Demand Draft" {{ old('payment_mode', $propertyMaster->payment_mode) === 'Demand Draft' ? 'selected' : '' }}>📜 Demand Draft (DD)</option>
                            <option value="Other" {{ old('payment_mode', $propertyMaster->payment_mode) === 'Other' ? 'selected' : '' }}>✨ Other</option>
                        </select>
            ...
            <div class="form-group full-width" style="background: linear-gradient(135deg, rgba(167, 139, 250, 0.08) 0%, rgba(139, 92, 246, 0.03) 100%); border: 1.5px solid rgba(167, 139, 250, 0.30); border-radius: 14px; padding: 18px; margin-bottom: 10px;">
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
                            ...
                            @if(isset($brokers))
                                @foreach($brokers as $b)
                                    <option value="{{ $b->id }}" 
                                            ...
                                            {{ old('broker_id', $propertyMaster->broker_id) == $b->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <input type="text" name="broker_name" id="pm_broker_name" value="{{ old('broker_name', $propertyMaster->broker_name) }}" class="form-control" placeholder="e.g. Ramesh Patel">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <input type="text" name="broker_notes" value="{{ old('broker_notes', $propertyMaster->broker_notes) }}" class="form-control" placeholder="e.g. 1% commission on registry">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-top: 14px; padding-top: 14px; border-top: 1px dashed rgba(167, 139, 250, 0.25);">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <div style="display: grid; grid-template-columns: 105px 1fr; gap: 8px;">
                            <select name="broker_commission_type" id="pm_broker_commission_type" class="form-control" onchange="recalculateBrokerage()">
                                <option value="percentage" {{ old('broker_commission_type', $propertyMaster->broker_commission_type ?? 'percentage') === 'percentage' ? 'selected' : '' }}>% Rate</option>
                                <option value="fixed" {{ old('broker_commission_type', $propertyMaster->broker_commission_type) === 'fixed' ? 'selected' : '' }}>Fixed ₹</option>
                            </select>
                            <div class="input-luxury-group border-purple">
                                <input type="number" step="0.01" name="broker_commission_rate" id="pm_broker_commission_rate" value="{{ old('broker_commission_rate', $propertyMaster->broker_commission_rate) }}" placeholder="Rate" oninput="recalculateBrokerage()">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <div class="input-luxury-group border-purple">
                            ...
                            <input type="number" step="0.01" name="broker_commission_amount" id="pm_broker_commission_amount" value="{{ old('broker_commission_amount', $propertyMaster->broker_commission_amount) }}" placeholder="0.00" oninput="recalculateBrokerageFromAmount()">
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <div class="input-luxury-group border-emerald">
                            <span class="input-luxury-addon text-emerald">₹</span>
                            <input type="number" step="0.01" name="broker_commission_paid" id="pm_broker_commission_paid" value="{{ old('broker_commission_paid', $propertyMaster->broker_commission_paid ?? 0) }}" placeholder="0.00" oninput="recalculateBrokerageDue()">
                        </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-top: 14px;">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <div class="input-luxury-group border-amber bg-readonly">
                            <span class="input-luxury-addon text-amber">₹</span>
                            <input type="number" step="0.01" name="broker_commission_due" id="pm_broker_commission_due" value="{{ old('broker_commission_due', $propertyMaster->broker_commission_due ?? 0) }}" placeholder="0.00" readonly>
                        </div>
                        <small style="color: #FDE68A; font-size: 11.5px; margin-top: 4px; display: block;">Auto-calculated (Total - Paid)</small>
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <select name="broker_commission_payment_mode" class="form-control">
                            ...
                            <option value="Cash" {{ old('broker_commission_payment_mode', $propertyMaster->broker_commission_payment_mode) === 'Cash' ? 'selected' : '' }}>💵 Cash</option>
                            <option value="Cheque" {{ old('broker_commission_payment_mode', $propertyMaster->broker_commission_payment_mode) === 'Cheque' ? 'selected' : '' }}>📝 Cheque</option>
                            <option value="Bank Transfer / RTGS" {{ old('broker_commission_payment_mode', $propertyMaster->broker_commission_payment_mode) === 'Bank Transfer / RTGS' ? 'selected' : '' }}>🏦 Bank Transfer / RTGS</option>
                            <option value="UPI" {{ old('broker_commission_payment_mode', $propertyMaster->broker_commission_payment_mode) === 'UPI' ? 'selected' : '' }}>📱 UPI (GPay/PhonePe)</option>
                            <option value="Other" {{ old('broker_commission_payment_mode', $propertyMaster->broker_commission_payment_mode) === 'Other' ? 'selected' : '' }}>✨ Other</option>
                        </select>
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <select name="broker_commission_status" id="pm_broker_commission_status" class="form-control" style="font-weight: 700;">
                            <option value="unpaid" {{ old('broker_commission_status', $propertyMaster->broker_commission_status ?? 'unpaid') === 'unpaid' ? 'selected' : '' }}>Unpaid (0%)</option>
                            <option value="partial" {{ old('broker_commission_status', $propertyMaster->broker_commission_status) === 'partial' ? 'selected' : '' }}>Partial</option>
                            <option value="paid" {{ old('broker_commission_status', $propertyMaster->broker_commission_status) === 'paid' ? 'selected' : '' }}>Paid (100% Full)</option>
                        </select>
            ...
            <div class="form-group">
                ...
                <input type="text" name="location" value="{{ old('location', $propertyMaster->location) }}" class="form-control" placeholder="e.g. Zadeshwar Road">
                @error('location') <span class="invalid-feedback">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">City</label>
                <input type="text" name="city" value="{{ old('city', $propertyMaster->city) }}" class="form-control" placeholder="e.g. Bharuch">
                @error('city') <span class="invalid-feedback">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">State</label>
                <input type="text" name="state" value="{{ old('state', $propertyMaster->state) }}" class="form-control" placeholder="e.g. Gujarat">
                @error('state') <span class="invalid-feedback">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">Country</label>
                <input type="text" name="country" value="{{ old('country', $propertyMaster->country ?? 'India') }}" class="form-control">
            ...
            <div class="form-group full-width">
                ...
                <textarea name="address" class="form-control" rows="2" placeholder="Full property address">{{ old('address', $propertyMaster->address) }}</textarea>
                @error('address') <span class="invalid-feedback">{{ $message }}</span> @enderror
            </div>

            <div class="form-group full-width">
                <label class="form-label">Description / Notes</label>
                <textarea name="description" class="form-control" rows="2" placeholder="Property details or notes">{{ old('description', $propertyMaster->description) }}</textarea>
...
function handlePropertyTypeChange(type) {
    ...
    if (t === 'land') {
        ...
        if (autoGenText) autoGenText.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing sub-plots on save';
        ...
        if (autoGenText) autoGenText.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing flats/units on save';
        ...
        if (autoGenText) autoGenText.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing ${labelName.toLowerCase()} units on save`;
        ...
        if (autoGenText) autoGenText.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing ${labelName.toLowerCase()}s on save`;
        ...
        if (autoGenText) autoGenText.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing farmhouse plots on save';
        ...
        if (unitListInput && !unitListInput.value) unitListInput.placeholder = 'e.g. 1-10, 30, 35 or 1 to 10, 30, 35';
        if (unitPrefixInput && (!unitPrefixInput.value || ['Plot ', 'Flat ', 'Shop ', 'House '].includes(unitPrefixInput.value))) unitPrefixInput.value = 'Plot ';
        if (autoGenText) autoGenText.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-create any missing sub-plots/units on save';
    }
