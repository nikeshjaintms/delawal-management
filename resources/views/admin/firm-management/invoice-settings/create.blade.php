...
<div class="form-card">
    ...
    <div class="form-grid-2">
        ...
        <div class="form-group">
            ...
            <input type="number" name="starting_number" value="{{ old('starting_number',1) }}" class="form-control @error('starting_number') is-invalid @enderror" min="1" required>
            <div class="form-hint">The first number in the series (e.g. 1)</div>
            @error('starting_number')<div class="text-error">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label class="form-label">Current Number <span>*</span></label>
            <input type="number" name="current_number" value="{{ old('current_number',1) }}" class="form-control @error('current_number') is-invalid @enderror" min="1" required>
            <div class="form-hint">Current counter value (usually same as starting)</div>
            @error('current_number')<div class="text-error">{{ $message }}</div>@enderror
        </div>
    </div>
</div>
...
<div class="form-card">
    ...
    <div class="form-grid">
        @foreach([
            ['sales_prefix','SAL','Sales Invoice'],
            ['purchase_prefix','PUR','Purchase Invoice'],
            ['booking_prefix','BKG','Booking'],
            ['rental_prefix','RNT','Rental'],
            ['payment_prefix','PAY','Payment'],
            ['receipt_prefix','REC','Receipt'],
            ['expense_prefix','EXP','Expense'],
            ['income_prefix','INC','Income'],
            ['loan_prefix','LAN','Loan'],
        ] as [$field, $default, $label])
        <div class="form-group">
            ...
            @error($field)<div class="text-error">{{ $message }}</div>@enderror
        </div>
        @endforeach
    </div>
...
<div class="form-action-buttons">
    <button type="submit" class="btn-primary-custom"><i class="fa fa-save"></i> Save Settings</button>
