...
/* total bar */
.total-bar{background:linear-gradient(135deg,rgba(16,185,129,0.06),rgba(16,185,129,0.02));border:1px solid rgba(16,185,129,0.2);border-radius:10px;padding:14px 18px;margin-bottom:18px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;}
.total-bar .tl{font-size:12.5px;color:#64748B;}
.total-bar .tv{font-size:19px;font-weight:800;color:#059669;}
...
.premium-table tbody tr:hover{background:#F0FDF4;}
.premium-table tr:last-child td{border-bottom:none;}
/* badges */
.cn-badge{display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase;}
...
<div class="crud-header">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
        <a href="{{ route('reports.credit-note') }}" style="padding:10px 18px;border:1px solid #E2E8F0;border-radius:8px;font-size:13.5px;font-weight:600;color:#64748B;background:#FFF;text-decoration:none;display:inline-flex;align-items:center;gap:7px;transition:all .18s;">
...
<div class="card-box">
    <form method="GET" action="{{ route('credit-notes.index') }}" class="filter-bar">
        ...
        @if(request()->hasAny(['search','filter_customer','filter_status','from_date','to_date']))
            <a href="{{ route('credit-notes.index') }}" class="btn-reset"><i class="fa-solid fa-rotate-left"></i> Reset</a>
        @endif
    </form>

    <div class="total-bar">
        <i class="fa-solid fa-circle-plus" style="color:#10B981;font-size:18px;"></i>
    ...
    <div class="table-wrap">
        <table class="premium-table">
            <thead>
                <tr>
                    <th>#</th><th>Credit Note No</th><th>Date</th><th>Customer</th>
            ...
            <tbody>
                ...
                <tr>
                    <td style="color:#94A3B8;font-size:12px;">{{ $creditNotes->firstItem() + $key }}</td>
                    <td style="font-weight:700;">{{ $cn->credit_note_no ?? '—' }}</td>
                    <td style="white-space:nowrap;font-size:13px;">{{ \Carbon\Carbon::parse($cn->credit_note_date)->format('d M Y') }}</td>
                    <td>
                        <div style="font-weight:600;font-size:13px;">{{ $cn->customer?->name ?? '—' }}</div>
                        @if($cn->customer?->mobile)<div style="font-size:11px;color:#64748B;">{{ $cn->customer->mobile }}</div>@endif
                    </td>
                    ...
                    <td style="text-align:right;font-size:13px;">₹{{ number_format($cn->taxable_amount,2) }}</td>
                    <td style="text-align:right;color:#EF4444;font-weight:700;">₹{{ number_format($cn->total_gst,2) }}</td>
                    <td style="text-align:right;color:#059669;font-weight:800;font-size:14px;">₹{{ number_format($cn->credit_amount,2) }}</td>
