...
<div class="crud-header">
    <div class="crud-title">
        <h2>Edit Labour: {{ $labour->name }}</h2>
        <p>Update worker details, compensation rates, or assign to another farm.</p>
    </div>
    <a href="{{ route('agriculture.labours.show', $labour->id) }}" class="btn-outline"><i class="fa-solid fa-arrow-left"></i> Back to Labour Ledger</a>
</div>

<div class="card-box">
    <form method="POST" action="{{ route('agriculture.labours.update', $labour->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $labour->name) }}" class="form-control @error('name') is-invalid @enderror" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="mobile_number" value="{{ old('mobile_number', $labour->mobile_number) }}" class="form-control @error('mobile_number') is-invalid @enderror">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="firm_id" class="form-control @error('firm_id') is-invalid @enderror">
                        @foreach($firms as $f)
                            <option value="{{ $f->id }}" {{ old('firm_id', $labour->firm_id) == $f->id ? 'selected' : '' }}>{{ $f->firm_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="farm_id" class="form-control @error('farm_id') is-invalid @enderror">
                        ...
                        @foreach($farms as $farm)
                            <option value="{{ $farm->id }}" {{ old('farm_id', $labour->farm_id) == $farm->id ? 'selected' : '' }}>{{ $farm->farm_name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="field_crop" value="{{ old('field_crop', $labour->field_crop) }}" class="form-control @error('field_crop') is-invalid @enderror">
                    @error('field_crop')<div class="text-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Joining Date</label>
                    <input type="date" name="joining_date" value="{{ old('joining_date', $labour->joining_date ? \Carbon\Carbon::parse($labour->joining_date)->format('Y-m-d') : '') }}" class="form-control @error('joining_date') is-invalid @enderror">
        ...
        <div class="form-section">
            ...
            <div class="form-row-3">
                <div class="form-group">
                    ...
                    <select name="labour_type" id="labour_type" class="form-control @error('labour_type') is-invalid @enderror" onchange="onLabourTypeChange(this.value)">
                        @foreach($labourTypes as $lt)
                            <option value="{{ $lt }}" {{ old('labour_type', $labour->labour_type) == $lt ? 'selected' : '' }}>{{ $lt }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" id="group_daily_wage">
                    ...
                    <input type="number" step="0.01" name="daily_wage" id="daily_wage" value="{{ old('daily_wage', $labour->daily_wage) }}" class="form-control @error('daily_wage') is-invalid @enderror">
                ...
                <div class="form-group" id="group_fixed_salary" style="display:none;">
                    ...
                    <input type="number" step="0.01" name="fixed_salary" id="fixed_salary" value="{{ old('fixed_salary', $labour->fixed_salary) }}" class="form-control @error('fixed_salary') is-invalid @enderror">
            ...
            <div class="form-group">
                ...
                <textarea name="notes" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $labour->notes) }}</textarea>
                @error('notes')<div class="text-error">{{ $message }}</div>@enderror
            </div>
        </div>

        <div class="form-actions">
            <button type="submit" class="btn-gold"><i class="fa-solid fa-check"></i> Update Labour Record</button>
            <a href="{{ route('agriculture.labours.show', $labour->id) }}" class="btn-outline">Cancel</a>
        </div>
    </form>
</div>
