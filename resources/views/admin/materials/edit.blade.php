...
<div class="crud-header">
    <div class="crud-title">
        <h2>Edit Material</h2>
        <p>Update — <strong>{{ $material->material_name }}</strong></p>
    </div>
</div>
...
<div class="card-box">
    <form method="POST" action="{{ route('materials.update', $material->id) }}" autocomplete="off">
        ...
        <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <select name="material_category_id" id="material_category_id" class="form-control @error('material_category_id') is-invalid @enderror">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" data-name="{{ $cat->category_name }}" {{ old('material_category_id', $material->material_category_id) == $cat->id ? 'selected' : '' }}>
                                {{ $cat->category_name }}
                            </option>
                        @endforeach
            ...
            <input type="hidden" name="specification" id="specification" value="{{ old('specification', $material->specification) }}">
        </div>
                    ...
                    <select name="project_id" id="project_id" class="form-control @error('project_id') is-invalid @enderror">
                        ...
                        @if(isset($projects))
                            @foreach($projects as $proj)
                                <option value="{{ $proj->id }}" {{ old('project_id', $material->project_id) == $proj->id ? 'selected' : '' }}>
                                    {{ $proj->project_name }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <select name="contractor_id" id="contractor_id" class="form-control @error('contractor_id') is-invalid @enderror">
                        ...
                        @foreach($contractors as $con)
                            <option value="{{ $con->id }}"
                                    ...
                                    {{ old('contractor_id', $material->contractor_id) == $con->id ? 'selected' : '' }}>
        ...
        <div class="form-section">
            ...
            <div class="form-row-4">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.001" min="0" name="opening_stock" id="opening_stock" value="{{ old('opening_stock', $material->opening_stock) }}" class="form-control @error('opening_stock') is-invalid @enderror" required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0" name="unit_price" id="unit_price" value="{{ old('unit_price', $material->unit_price ?? 0) }}" class="form-control @error('unit_price') is-invalid @enderror" placeholder="0.00">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0" name="total_price" id="total_price" value="{{ old('total_price', $material->total_price ?? (($material->opening_stock ?? 0) * ($material->unit_price ?? 0))) }}" class="form-control @error('total_price') is-invalid @enderror" placeholder="0.00" readonly style="background: rgba(16, 22, 34, 0.90) !important; color: #60A5FA !important; font-weight: 700;">
                    <div class="form-hint">Qty × Unit Price.</div>
                    @error('total_price')<div class="text-error">{{ $message }}</div>@enderror
                </div>
            </div>

            <input type="hidden" name="minimum_stock" id="minimum_stock" value="{{ $material->minimum_stock ?? 0 }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize on load
    updatePresetDropdown();
    const initialContractorId = "{{ old('contractor_id', $material->contractor_id) }}";
