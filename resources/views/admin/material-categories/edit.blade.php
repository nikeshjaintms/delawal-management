...
<div class="crud-header"><div class="crud-title"><h2>Edit Material Category</h2><p>Update — <strong>{{ $materialCategory->category_name }}</strong></p></div></div>
<div class="card-box">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                    <option value="active" {{ old('status',$materialCategory->status)=='active'?'selected':'' }}>Active</option>
                    <option value="inactive" {{ old('status',$materialCategory->status)=='inactive'?'selected':'' }}>Inactive</option>
                </select>
            </div>
        </div>
        <div class="form-group">
            <label class="form-label" for="description">Description</label>
            <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror">{{ old('description',$materialCategory->description) }}</textarea>
        </div>
        <div class="form-actions">
            <button type="submit" class="btn-gold"><i class="fa-solid fa-floppy-disk"></i> Update Category</button>
            <a href="{{ route('material-categories.index') }}" class="btn-outline">Back</a>
        </div>
    </form>
</div>
@endsection
