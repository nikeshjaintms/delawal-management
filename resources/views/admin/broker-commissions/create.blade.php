            ...
            <div class="form-group">
                ...
                <select name="broker_id" id="broker_id" class="form-control @error('broker_id') is-invalid @enderror">
                    ...
                    @foreach($brokers as $b)
                        <option value="{{ $b->id }}" {{ old('broker_id') == $b->id ? 'selected' : '' }} data-commission="{{ $b->commission_percentage }}">{{ $b->name }} ({{ $b->mobile }})</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="property_id" id="property_id" class="form-control @error('property_id') is-invalid @enderror">
                    ...
                    @foreach($properties as $p)
                        <option value="{{ $p->id }}" data-project="{{ $p->project->project_name ?? ($p->project->propertyMaster->property_name ?? 'No Project Assigned') }}" {{ old('property_id') == $p->id ? 'selected' : '' }}>{{ $p->property_name }} (₹{{ number_format($p->price, 0) }})</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="booking_id" id="booking_id" class="form-control @error('booking_id') is-invalid @enderror">
                    ...
                    @foreach($bookings as $bk)
                        <option value="{{ $bk->id }}" {{ old('booking_id') == $bk->id ? 'selected' : '' }} data-property="{{ $bk->property_id }}" data-customer="{{ $bk->customer_id }}">
...
<script>
    ...
    // Auto-fill broker default commission percentage if percentage is selected
