    ...
    <form method="POST" action="{{ route('loans.update', $loan->id) }}" id="loanForm">
        ...
        <div class="form-section">
            <label class="emi-toggle-card" for="has_emi">
                <input type="checkbox" name="has_emi" id="has_emi" value="1" {{ old('has_emi', $loan->has_emi) ? 'checked' : '' }} onchange="toggleEmiSection()">
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="loan_start_date" id="loan_start_date" value="{{ old('loan_start_date', $loan->loan_start_date ? \Carbon\Carbon::parse($loan->loan_start_date)->format('Y-m-d') : '') }}" class="form-control @error('loan_start_date') is-invalid @enderror" onchange="calcEmi()">
            ...
            <div id="emi_fields_wrapper">
                ...
                <div class="form-group" style="display:flex;align-items:center;gap:10px;margin-top:10px;background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.25);border-radius:12px;padding:12px 16px;">
                    <input type="checkbox" name="regenerate_emi" id="regenerate_emi" value="1" style="width:18px;height:18px;cursor:pointer;">
                    <label for="regenerate_emi" style="font-size:13.5px;font-weight:700;color:#F87171;cursor:pointer;">Regenerate EMI Schedule (Overwrites existing pending schedule)</label>
                </div>
                @endif
            </div>
        </div>
...
function toggleEmiSection() {
    ...
    const emiInputs = emiWrapper.querySelectorAll('input:not(#regenerate_emi)');
...
function updateSubmitButton() {
    ...
    if (hasEmi) {
        submitBtn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Update Loan & EMI';
    } else {
        submitBtn.innerHTML = loanType === 'Personal Loan'
            ? '<i class="fa-solid fa-floppy-disk"></i> Update Personal Loan'
            : '<i class="fa-solid fa-floppy-disk"></i> Update Loan';
    }
}
