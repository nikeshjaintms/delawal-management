...
<div class="crud-header">
    <div class="crud-title"><h2>Edit Ledger Entry</h2><p>Update — <strong>{{ $ledger->transaction_title }}</strong></p></div>
</div>

<div class="card-box">
    <form method="POST" action="{{ route('ledgers.update', $ledger->id) }}">
        @csrf @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="transaction_type" class="form-control @error('transaction_type') is-invalid @enderror">
                        ...
                        @foreach(['Sale','Payment Received','Expense','Purchase','Rent Received','Loan EMI','Other'] as $t)
                            <option value="{{ $t }}" {{ old('transaction_type',$ledger->transaction_type)==$t?'selected':'' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="payment_mode" class="form-control @error('payment_mode') is-invalid @enderror">
                        ...
                        @foreach($paymentModes as $pm)
                            <option value="{{ $pm->name }}" {{ old('payment_mode',$ledger->payment_mode)==$pm->name?'selected':'' }}>{{ $pm->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select name="property_id" id="property_id" class="form-control @error('property_id') is-invalid @enderror">
                        ...
                        @foreach($properties as $p)
                            <option value="{{ $p->id }}" data-project="{{ $p->project->project_name ?? ($p->project->propertyMaster->property_name ?? 'No Project Assigned') }}" {{ old('property_id',$ledger->property_id)==$p->id?'selected':'' }}>{{ $p->property_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="vendor_id" class="form-control @error('vendor_id') is-invalid @enderror">
                        ...
                        @foreach($vendors as $v)
                            <option value="{{ $v->id }}" {{ old('vendor_id',$ledger->vendor_id)==$v->id?'selected':'' }}>{{ $v->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="broker_id" class="form-control @error('broker_id') is-invalid @enderror">
                        ...
                        @foreach($brokers as $b)
                            <option value="{{ $b->id }}" {{ old('broker_id',$ledger->broker_id)==$b->id?'selected':'' }}>{{ $b->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                <textarea name="remarks" class="form-control @error('remarks') is-invalid @enderror" placeholder="Additional notes...">{{ old('remarks',$ledger->remarks) }}</textarea>
            </div>
        </div>

        <div class="form-actions">
            <button type="submit" class="btn-gold"><i class="fa-solid fa-floppy-disk"></i> Update Entry</button>
            <a href="{{ route('ledgers.show', $ledger->id) }}" class="btn-outline"><i class="fa-solid fa-arrow-left"></i> Back</a>
        </div>
    </form>
</div>
