...
<div class="crud-header">
    <div class="crud-title">
        <h2>Payment Mode Master</h2>
        <p>Create and manage payment methods used in transactions.</p>
    </div>
    <a href="{{ route('payment-modes.create') }}" class="btn-gold">
...
<div class="card-box">
    <div class="filter-bar">
        <form method="GET" action="{{ route('payment-modes.index') }}" class="search-form">
            <input type="text" name="search" value="{{ request('search') }}"
                   placeholder="Search by name, description, status..." class="search-input @error('search') is-invalid @enderror">
            <button type="submit" class="btn-search">Search</button>
            @if(request('search'))
                <a href="{{ route('payment-modes.index') }}" class="btn-reset">Reset</a>
            @endif
        </form>
    </div>
            ...
            <tbody>
                @forelse($paymentModes as $key => $mode)
                    <tr>
                        ...
                        <td>
                            @if($mode->description)
                                <span class="desc-cell" title="{{ $mode->description }}">{{ $mode->description }}</span>
                            @else
                                <span style="color:var(--text-secondary);">-</span>
                            @endif
                        </td>
                        <td>
                            <span class="badge badge-{{ $mode->status }}">{{ ucfirst($mode->status) }}</span>
                        </td>
                        <td>
                            <div class="table-action-buttons">
                                <a href="{{ route('payment-modes.show', $mode->id) }}" class="btn-view">
                                    <i class="fa fa-eye"></i> View
                                </a>
                                <a href="{{ route('payment-modes.edit', $mode->id) }}" class="btn-edit">
                                    <i class="fa fa-edit"></i> Edit
                                </a>
                                <form action="{{ route('payment-modes.destroy', $mode->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        onclick="return confirm('Are you sure you want to delete this payment mode?')"
                                        class="btn-delete">
