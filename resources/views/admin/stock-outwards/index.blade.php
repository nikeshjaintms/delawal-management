...
<style>
    ...
    .filter-control{padding:9px 12px;border:1px solid var(--border-color);border-radius:8px;font-size:13px;font-family:var(--font-primary);outline:none;background:#FFF;min-width:150px;}
    ...
    .qty-chip{background:rgba(239,68,68,0.1);color:#B91C1C;padding:3px 9px;border-radius:6px;font-size:12px;font-weight:700;display:inline-block;}
...
<div class="crud-header">
    <div class="crud-title"><h2>Stock Outward</h2><p>Record material usage and stock issues.</p></div>
...
<div class="card-box">
    <form method="GET" action="{{ route('stock-outwards.index') }}" class="filter-bar">
        <div class="filter-group">
            ...
            <input type="text" name="search" value="{{ request('search') }}" class="search-input @error('search') is-invalid @enderror" placeholder="Material, used for, property...">
        </div>
        <div class="filter-group">
    ...
    <div class="table-container">
        <table class="premium-table">
            <thead>
                <tr><th>No</th><th>Date</th><th>Material</th><th>Property</th><th>Quantity Issued</th><th>Used For</th><th>Remarks</th><th style="width:170px;">Action</th></tr>
            </thead>
            <tbody>
                @forelse($outwards as $key => $out)
                <tr>
                    <td>{{ $outwards->firstItem() + $key }}</td>
                    <td style="white-space:nowrap;">{{ \Carbon\Carbon::parse($out->outward_date)->format('d M Y') }}</td>
                    <td>
                        <strong>{{ $out->material->material_name ?? '-' }}</strong>
                        @if($out->material?->materialCategory)
                            <div style="font-size:11px;color:var(--text-secondary);">{{ $out->material->materialCategory->category_name }}</div>
                        @endif
                    </td>
                    <td>{{ $out->property->property_name ?? '<span style="color:var(--text-secondary);">-</span>' }}</td>
                    <td><span class="qty-chip">-{{ number_format($out->quantity,3) }} {{ $out->material?->unit }}</span></td>
                    <td>{{ $out->used_for ?? '-' }}</td>
                    <td style="font-size:12.5px;color:var(--text-secondary);max-width:150px;">{{ $out->remarks ? \Illuminate\Support\Str::limit($out->remarks,40) : '-' }}</td>
                    <td>
                        <div class="table-action-buttons">
                            ...
                            <form action="{{ route('stock-outwards.destroy', $out->id) }}" method="POST" style="display:inline;" id="del-out-{{ $out->id }}">
                                @csrf @method('DELETE')
                                <button type="button" class="btn-delete" onclick="confirmDel({{ $out->id }},'{{ addslashes($out->material?->material_name ?? '') }}','del-out-')"><i class="fa fa-trash"></i> Delete</button>
                            </form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="8" align="center" style="padding:30px;color:var(--text-secondary);">No stock outward records found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="pagination-wrapper">{{ $outwards->appends(request()->query())->links() }}</div>
</div>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
function confirmDel(id,name,prefix){
    Swal.fire({title:'Delete Outward?',html:'Delete outward record for <strong>'+name+'</strong>?<br><small style="color:#64748B;">Stock will be reversed (added back).</small>',icon:'warning',showCancelButton:true,confirmButtonColor:'#EF4444',cancelButtonColor:'#64748B',confirmButtonText:'Yes, Delete',cancelButtonText:'Cancel',customClass:{popup:'swal-inv-popup'}})
