...
.badge-imir { background: rgba(236, 72, 153, 0.15) !important; color: #F472B6 !important; border: 1px solid rgba(244, 114, 182, 0.30) !important; font-weight: 600; }
    ...
    <div class="table-responsive-wrapper">
        <table class="premium-table">
            ...
            <tbody>
                ...
                @endphp
                <tr>
                    <td><strong style="color: #FFFFFF !important; font-weight: 700;">{{ $out->outward_number ?: 'SO-'.$out->id }}</strong></td>
                    <td>{{ $out->outward_date->format('d M Y') }}</td>
                    <td>
                        @if($out->stock_inward_number)
                            <span class="badge badge-imir"><i class="fa-solid fa-boxes-packing" style="font-size:10px;"></i> {{ $out->stock_inward_number }}</span>
                        @else
                    ...
                    <td style="text-align:right;">
                        <div class="table-action-buttons">
                            <a href="{{ route('stock-outwards.show', $out->id) }}" class="btn-view"><i class="fa-solid fa-eye"></i> View</a>
                            <a href="{{ route('stock-outwards.print', $out->id) }}" target="_blank" class="btn-print"><i class="fa-solid fa-print"></i> Print</a>
                            @if(!$out->stock_inward_number)
                            <a href="{{ route('stock-outwards.edit', $out->id) }}" class="btn-edit"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                            @endif
...
function confirmDel(id,name,prefix){
    Swal.fire({
        ...
        html:'Are you sure you want to delete transaction <strong>'+name+'</strong>?<br><small style="color:#94A3B8;">All related items will be deleted and stock will be restored.</small>',
        icon:'warning',
