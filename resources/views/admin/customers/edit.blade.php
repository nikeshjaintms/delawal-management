...
<div class="crud-header">
    <div class="crud-title">
        <h2>Edit Customer</h2>
        <p>Update customer details — <strong>{{ $customer->name }}</strong></p>
    </div>
</div>
...
<div class="card-box">
    <form method="POST" action="{{ route('customers.update', $customer->id) }}">
        @csrf
        @method('PUT')
        @include('admin.components.firm-select', ['model' => $customer])

        <div class="form-row">
            <div class="form-group">
                <label class="form-label" for="name">Customer Name <span>*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name', $customer->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="Enter customer name">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="customer_type" id="customer_type" class="form-control @error('customer_type') is-invalid @enderror">
                    ...
                    <option value="rental" {{ old('customer_type', $customer->customer_type) == 'rental' ? 'selected' : '' }}>Rental</option>
                </select>
