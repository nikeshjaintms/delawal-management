...
.filter-ctrl{padding:9px 12px;border:1.5px solid #E2E8F0;border-radius:8px;font-size:13px;font-family:inherit;outline:none;background:#fff;transition:border-color .18s ease;min-width:150px;}
.filter-ctrl:focus{border-color:#3B82F6;box-shadow:0 0 0 3px rgba(59,130,246,0.15);}
.btn-filter{background:#0F172A;color:#fff;padding:9px 16px;border-radius:8px;border:none;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;align-self:flex-end;display:inline-flex;align-items:center;gap:6px;transition:background .18s ease;}
.btn-filter:hover{background:#1E293B;}
.btn-reset{padding:9px 10px;color:#64748B;text-decoration:none;font-size:13px;align-self:flex-end;display:inline-flex;align-items:center;gap:5px;transition:color .18s ease;}
.btn-reset:hover{color:#0F172A;}
/* Table */
.table-wrap{width:100%;overflow-x:auto;}
.gst-table{width:100%;border-collapse:collapse;font-size:13px;}
.gst-table thead th{padding:11px 13px;background:#F8FAFC;color:#475569;font-weight:700;border-bottom:2px solid #E2E8F0;font-size:11px;text-transform:uppercase;letter-spacing:.7px;white-space:nowrap;}
...
/* Badges */
.status-badge{display:inline-block;padding:3px 9px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase;}
...
.rpt-footer-bar{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-top:16px;padding-top:14px;border-top:1px solid #F1F5F9;}
...
<div class="rpt-header">
    <div class="rpt-title-block">
        <h2><i class="fa-solid fa-cart-flatbed" style="color:#F97316;margin-right:9px;"></i>GST Purchase Report</h2>
...
<div class="gst-stat-grid">
    <div class="gst-stat-card">
        ...
        <div class="sc-value">{{ $totalBills }}</div>
    </div>
    <div class="gst-stat-card">
...
<div class="card-box">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
        <div style="font-size:13.5px;font-weight:700;color:#0F172A;">
            <i class="fa-solid fa-table-list" style="color:#F97316;margin-right:7px;"></i>
        ...
        @if(request()->hasAny(['from_date','to_date','filter_vendor','filter_category','filter_status']))
            <span style="font-size:12px;color:#64748B;display:flex;align-items:center;gap:5px;">
                <i class="fa-solid fa-filter" style="color:#F97316;"></i> Filtered results
            </span>
        @endif
    </div>
        ...
        <table class="gst-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight:600;font-size:13px;">{{ $exp->bill_no ?? '—' }}</td>
                    <td style="font-size:13px;">{{ $exp->invoice_no ?? '—' }}</td>
                    ...
                    <td>
                        ...
                        @if($exp->vendor?->mobile)
                            <div style="font-size:11px;color:#64748B;">{{ $exp->vendor->mobile }}</div>
                        @endif
                    </td>
                    <td>
                        <div style="font-size:13px;font-weight:500;">{{ $exp->expense_title }}</div>
                        @if($exp->expense_category)
                            <div style="font-size:11px;color:#64748B;">{{ $exp->expense_category }}</div>
                        @endif
                    </td>
                    <td>
                        @if($exp->hsn_code)
                    ...
                    <td class="amt">₹{{ number_format($exp->computed_taxable,2) }}</td>
                    <td class="amt amt-gst" style="color:#0EA5E9;">
                        ₹{{ number_format($exp->computed_cgst,2) }}
                        @if($exp->cgst_rate)
                            <div style="font-size:11px;color:#94A3B8;font-weight:400;">({{ $exp->cgst_rate }}%)</div>
                        @endif
                    </td>
                    <td class="amt amt-gst" style="color:#14B8A6;">
                        ₹{{ number_format($exp->computed_sgst,2) }}
                        @if($exp->sgst_rate)
                            <div style="font-size:11px;color:#94A3B8;font-weight:400;">({{ $exp->sgst_rate }}%)</div>
                        @endif
                    </td>
                    <td class="amt amt-gst" style="color:#8B5CF6;">
                        ₹{{ number_format($exp->computed_igst,2) }}
                        @if($exp->igst_rate)
                            <div style="font-size:11px;color:#94A3B8;font-weight:400;">({{ $exp->igst_rate }}%)</div>
                        @endif
                    </td>
            ...
            <tfoot>
                <tr>
                    <td colspan="7" style="font-size:13px;color:#0F172A;">
                        <i class="fa-solid fa-sigma" style="color:#F97316;margin-right:6px;"></i>
