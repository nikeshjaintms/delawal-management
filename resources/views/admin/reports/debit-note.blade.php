...
.sc-crimson { background: rgba(225, 29, 72, 0.18) !important;  color: #FB7185 !important; border: 1px solid rgba(225, 29, 72, 0.35); }
...
.amt-grand { color: #FB7185 !important; font-weight: 800; font-size: 14.5px; }

/* Empty state */
.empty-state { text-align: center; padding: 52px 20px; color: #94A3B8; }
.empty-state i { font-size: 40px; margin-bottom: 14px; display: block; opacity: .4; color: #FB7185; }
...
<div class="rpt-header">
    <div class="rpt-title-block">
        <h2><i class="fa-solid fa-file-circle-minus" style="color:#FB7185;margin-right:9px;"></i>Debit Note Report</h2>
        <p>Purchase return and supplier adjustment GST summary with taxable amount, CGST, SGST, IGST breakup and total debit.</p>
    </div>
...
<div class="gst-stat-grid">
    ...
    <div class="gst-stat-card">
        ...
        <div class="sc-value" style="color:#FB7185 !important;">₹{{ number_format($totalDebit,2) }}</div>
    </div>
</div>
...
<div class="card-box">
    <form method="GET" action="{{ route('reports.debit-note') }}" class="filter-bar">
        ...
        @if(request()->hasAny(['from_date','to_date','filter_vendor']))
...
<div class="card-box">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;flex-wrap:wrap;gap:10px;">
        <div style="font-size:16px;font-weight:800;color:#FFFFFF !important;">
            <i class="fa-solid fa-table-list" style="color:#FB7185;margin-right:8px;"></i>
            Debit Note Records
        ...
        @if(request()->hasAny(['from_date','to_date','filter_vendor']))
            <span style="font-size:13px;font-weight:700;color:#FB7185;display:flex;align-items:center;gap:5px;">
                <i class="fa-solid fa-filter" style="color:#FB7185;"></i> Filtered results
            </span>
        @endif
    </div>
        ...
        <table class="gst-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight:700;font-size:13px;color:#FB7185;">{{ $n->debit_note_no }}</td>
                    <td style="white-space:nowrap;font-size:13px;color:#CBD5E1;font-weight:600;">
                        {{ \Carbon\Carbon::parse($n->note_date)->format('d M Y') }}
                    </td>
                    <td>
                        <div style="font-weight:700;font-size:13.5px;color:#FFFFFF;">{{ $n->vendor?->name ?? '—' }}</div>
                        @if($n->vendor?->mobile)
                            <div style="font-size:11.5px;color:#94A3B8;font-weight:500;">{{ $n->vendor->mobile }}</div>
                        @endif
                    </td>
                    <td style="font-size:13px;color:#60A5FA;font-weight:600;">{{ $n->original_bill_no ?? '—' }}</td>
                    ...
                    <td class="amt amt-grand" style="color:#FB7185 !important;">₹{{ number_format($n->total_amount,2) }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="12">
                        <div class="empty-state">
                            ...
                            <p>No debit note records found for the selected filters.</p>
                            @if(request()->hasAny(['from_date','to_date','filter_vendor']))
                                <a href="{{ route('reports.debit-note') }}"
                                   style="color:#FB7185;font-weight:700;font-size:13px;margin-top:8px;display:inline-block;text-decoration:none;">
                                    Clear all filters
                                </a>
                            @endif
                        </div>
            ...
            <tfoot>
                <tr>
                    <td colspan="6" style="font-size:13.5px;color:#FFFFFF !important;font-weight:800;">
                        <i class="fa-solid fa-calculator" style="color:#FB7185;margin-right:6px;"></i>
                    ...
                    <td class="amt amt-grand" style="font-size:15px;color:#FB7185 !important;font-weight:800;">₹{{ number_format($totalDebit,2) }}</td>
                </tr>
            </tfoot>
            @endif
        </table>
    ...
    <div class="rpt-footer-bar">
        <span style="font-size:13.5px;color:#FFFFFF !important;font-weight:700 !important;">
            ...
            &nbsp;·&nbsp; Total Debit: <strong style="color:#FB7185 !important;">₹{{ number_format($totalDebit,2) }}</strong>
            &nbsp;·&nbsp; Total GST: <strong style="color:#F87171 !important;">₹{{ number_format($totalGst,2) }}</strong>
        </span>
        <span style="font-size:13px;color:#FFFFFF !important;font-weight:700 !important;"><i class="fa-regular fa-clock" style="color:#FB7185;"></i> Generated: {{ now()->format('d M Y, h:i A') }}</span>
    </div>
    @endif
</div>
@endsection
