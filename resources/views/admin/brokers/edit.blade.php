...
<div class="crud-header">
    <div class="crud-title">
        <h2>Edit Broker</h2>
        <p>Update broker details — <strong>{{ $broker->name }}</strong></p>
    </div>
</div>
    ...
    <form method="POST" action="{{ route('brokers.update', $broker->id) }}">
        ...
        <div class="form-row">
            <div class="form-group">
                <label class="form-label" for="name">Broker Name <span>*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name', $broker->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="Enter broker name">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $broker->email) }}" class="form-control @error('email') is-invalid @enderror" placeholder="Enter email address">
                @error('email') <div class="text-error">{{ $message }}</div> @enderror
            </div>

            <div class="form-group">
                <label class="form-label" for="city">City</label>
                <input type="text" name="city" id="city" value="{{ old('city', $broker->city) }}" class="form-control @error('city') is-invalid @enderror" placeholder="Enter city name">
        ...
        <div class="form-actions">
            <button type="submit" class="btn-gold">
                <i class="fa-solid fa-floppy-disk"></i> Update Broker
            </button>
