...
<div class="crud-header">
    <div class="crud-title">
        <h2>Expense Category Master</h2>
        <p>Create and manage categories for business and project expenses.</p>
    </div>
    <a href="{{ route('expense-categories.create') }}" class="btn-gold">
...
<div class="card-box">
    <div class="filter-bar">
        <form method="GET" action="{{ route('expense-categories.index') }}" class="search-form">
            <input type="text" name="search" value="{{ request('search') }}"
                   placeholder="Search by name, description, status..." class="search-input @error('search') is-invalid @enderror">
            <button type="submit" class="btn-search">Search</button>
            @if(request('search'))
                <a href="{{ route('expense-categories.index') }}" class="btn-reset">Reset</a>
            @endif
        </form>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($category->description)
                                <span class="desc-cell" title="{{ $category->description }}">{{ $category->description }}</span>
                            @else
                                <span style="color:var(--text-secondary);">-</span>
                            @endif
                        </td>
                        <td>
                            <span class="badge badge-{{ $category->status }}">{{ ucfirst($category->status) }}</span>
                        </td>
                        <td>
                            <div class="table-action-buttons">
                                <a href="{{ route('expense-categories.show', $category->id) }}" class="btn-view">
                                    <i class="fa fa-eye"></i> View
                                </a>
                                <a href="{{ route('expense-categories.edit', $category->id) }}" class="btn-edit">
