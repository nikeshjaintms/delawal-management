...
.dn-icon {
    ...
    background: rgba(239, 68, 68, 0.15) !important; border: 1.5px solid rgba(239, 68, 68, 0.35) !important;
    display: flex; align-items: center; justify-content: center; font-size: 24px; color: #F87171 !important; flex-shrink: 0;
}
.dn-hero-info h3 { font-size: 21px; font-weight: 800; color: #FFFFFF !important; margin-bottom: 5px; }
.dn-hero-info p { font-size: 13.5px; color: #CBD5E1 !important; margin-bottom: 8px; }
.hero-badges { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }

.dn-badge { display: inline-flex; align-items: center; gap: 4px; padding: 5px 12px; font-size: 11px; font-weight: 700; border-radius: 20px; text-transform: uppercase; white-space: nowrap !important; }
.dn-approved { background: rgba(16, 185, 129, 0.18) !important; color: #34D399 !important; border: 1px solid rgba(16, 185, 129, 0.35) !important; }
.dn-pending { background: rgba(245, 158, 11, 0.18) !important; color: #FBBF24 !important; border: 1px solid rgba(245, 158, 11, 0.35) !important; }
.dn-rejected { background: rgba(239, 68, 68, 0.18) !important; color: #F87171 !important; border: 1px solid rgba(239, 68, 68, 0.35) !important; }
...
.detail-item:hover { border-color: rgba(239, 68, 68, 0.40) !important; background: rgba(22, 30, 46, 0.85) !important; }
...
.detail-label i { color: #F87171 !important; font-size: 11px; }
...
.meta-item i { color: #F87171 !important; }
...
.btn-red {
    ...
    display: inline-flex; align-items: center; gap: 8px; border: 1px solid #EF4444 !important;
    cursor: pointer; transition: all .25s ease; box-shadow: 0 4px 18px rgba(220,38,38,0.38);
}
...
<div class="card-box">
    ...
    <div class="dn-hero">
        ...
        <div class="dn-hero-info">
            ...
            <div class="hero-badges">
                <span style="font-size:21px;font-weight:800;color:#F87171;">₹{{ number_format($debitNote->debit_amount,2) }}</span>
    ...
    <div class="gst-summary">
        <div class="gst-row">
            ...
            <span style="font-weight:700;color:#FFFFFF;">₹{{ number_format($debitNote->taxable_amount,2) }}</span>
        </div>
        <div class="gst-row">
            <span style="color:#60A5FA;">CGST {{ $debitNote->cgst_rate ? '('.$debitNote->cgst_rate.'%)' : '' }}</span>
            <span style="font-weight:700;color:#60A5FA;">₹{{ number_format($debitNote->cgst_amount,2) }}</span>
        </div>
        <div class="gst-row">
            <span style="color:#2DD4BF;">SGST {{ $debitNote->sgst_rate ? '('.$debitNote->sgst_rate.'%)' : '' }}</span>
            <span style="font-weight:700;color:#2DD4BF;">₹{{ number_format($debitNote->sgst_amount,2) }}</span>
        </div>
        <div class="gst-row">
            <span style="color:#C4B5FD;">IGST {{ $debitNote->igst_rate ? '('.$debitNote->igst_rate.'%)' : '' }}</span>
            <span style="font-weight:700;color:#C4B5FD;">₹{{ number_format($debitNote->igst_amount,2) }}</span>
        </div>
        <div class="gst-row">
            <span style="color:#F87171;font-weight:600;">Total GST</span>
            <span style="font-weight:700;color:#F87171;">₹{{ number_format($debitNote->total_gst,2) }}</span>
        </div>
        <div class="gst-row" style="border-bottom:none;">
            <span style="font-size:15px;font-weight:800;color:#FFFFFF;">Debit Amount (Grand Total)</span>
            <span style="font-size:18px;font-weight:800;color:#F87171;">₹{{ number_format($debitNote->debit_amount,2) }}</span>
        </div>
    </div>
