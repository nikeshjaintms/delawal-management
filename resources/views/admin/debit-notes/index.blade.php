...
.total-bar .tv { font-size: 21px; font-weight: 800; color: #F87171 !important; }
...
<div class="crud-header">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
        <a href="{{ route('reports.debit-note') }}" class="btn-outline" style="padding:10px 18px;font-size:13.5px;font-weight:600;">
...
<div class="card-box">
    ...
    <div class="total-bar">
        <i class="fa-solid fa-circle-minus" style="color:#F87171;font-size:22px;"></i>
    ...
    <div class="table-wrap">
        <table class="premium-table">
            ...
            <tbody>
                ...
                <tr>
                    <td style="color:#94A3B8;font-size:12px;">{{ method_exists($debitNotes, 'firstItem') ? ($debitNotes->firstItem() + $key) : ($key + 1) }}</td>
                    <td><strong style="color: #FFFFFF !important;">{{ $dn->firm->firm_name ?? '—' }}</strong></td>
                    <td style="font-weight:700;color:#FFFFFF;">{{ $dn->debit_note_no ?? '—' }}</td>
                    <td style="white-space:nowrap;font-size:13px;color:#E2E8F0;">{{ \Carbon\Carbon::parse($dn->debit_note_date)->format('d M Y') }}</td>
                    <td>
                        <div style="font-weight:700;font-size:13.5px;color:#FFFFFF;">{{ $dn->vendor?->name ?? '—' }}</div>
                        @if($dn->vendor?->mobile)<div style="font-size:11.5px;color:#CBD5E1;">{{ $dn->vendor->mobile }}</div>@endif
                    </td>
                    <td style="font-size:13px;color:#CBD5E1;">{{ $dn->related_bill_no ?? '—' }}</td>
                    ...
                    <td style="text-align:right;font-size:13px;color:#E2E8F0;">₹{{ number_format($dn->taxable_amount,2) }}</td>
                    <td style="text-align:right;color:#F87171;font-weight:700;">₹{{ number_format($dn->total_gst,2) }}</td>
                    <td style="text-align:right;color:#F87171;font-weight:800;font-size:14.5px;">₹{{ number_format($dn->debit_amount,2) }}</td>
    ...
    @if(method_exists($debitNotes, 'links'))
