...
<head>
    ...
    <title>Property Sales Agreement Report - Delawala Management</title>
    <style>
...
<div class="rpt-header">
    ...
    <div class="rpt-meta">
        <div class="rpt-title">Property Sales Agreement Report</div>
...
<div class="stat-row">
    <div class="stat-box">
        <div class="s-label">Total Sales Agreements</div>
        <div class="s-value">{{ $totalSalesCount }}</div>
    </div>
    <div class="stat-box s-orange">
        <div class="s-label">Total Sales Value</div>
        <div class="s-value">₹{{ number_format($totalSaleAmount, 2) }}</div>
    </div>
    <div class="stat-box s-green">
        <div class="s-label">Total Received / Booking</div>
        <div class="s-value">₹{{ number_format($totalBookingAmount, 2) }}</div>
    </div>
    <div class="stat-box s-amber">
        <div class="s-label">Total Outstanding Due</div>
...
<table>
    <thead>
        <tr>
            ...
            <th class="r">Received / Booking</th>
            <th class="r">Remaining Due</th>
            <th class="c">Payment Status</th>
            <th class="c">Sale Status</th>
        </tr>
    </thead>
    <tbody>
        @forelse($propertySales as $i => $sale)
        <tr>
            ...
            <td>
                ...
                <span style="font-size:9.5px; color:#64748B;">{{ $sale->customer->phone ?? '' }}</span>
            </td>
            <td>{{ $sale->firm->firm_name ?? '-' }}</td>
            <td>{{ $sale->broker->name ?? '-' }}</td>
            <td>{{ $sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('d M Y') : '-' }}</td>
            <td class="r">₹{{ number_format($sale->sale_amount, 2) }}</td>
            <td class="r" style="color:#059669;">₹{{ number_format($sale->booking_amount, 2) }}</td>
            <td class="r" style="color:#B45309;">₹{{ number_format($sale->remaining_amount, 2) }}</td>
            <td class="c">
            ...
            <td class="c">
                ...
                @else
                    <span class="badge badge-danger">{{ ucfirst($sale->sale_status) }}</span>
                @endif
            </td>
        </tr>
        @empty
        <tr><td colspan="11" style="text-align:center;padding:20px;color:#64748B;">No property sales records found.</td></tr>
        @endforelse
    </tbody>
    @if($propertySales->count() > 0)
    <tfoot>
        <tr>
            <td colspan="6" style="font-size:11px;">Total ({{ $propertySales->count() }} sales)</td>
            <td class="r">₹{{ number_format($totalSaleAmount, 2) }}</td>
...
<div class="rpt-footer">
    <span>Delawala Management System &nbsp;—&nbsp; Property Sales Report</span>
    <span>{{ $propertySales->count() }} records &nbsp;|&nbsp; Total Sales: ₹{{ number_format($totalSaleAmount, 2) }} &nbsp;|&nbsp; {{ now()->format('d M Y') }}</span>
</div>
