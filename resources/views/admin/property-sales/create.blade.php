...
@section('title', 'Add Property Sell')
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <div class="form-hint">Select the main Property / Land acquisition first.</div>
            ...
            <div class="form-group" style="margin-bottom: 22px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                    <label id="scope_entire_card" style="display: flex; align-items: center; gap: 12px; padding: 14px 18px; border-radius: 12px; border: 1.5px solid rgba(59, 130, 246, 0.5); background: rgba(37, 99, 235, 0.12); cursor: pointer; transition: all .2s ease;">
                        <input type="radio" name="sale_scope" id="sale_scope_entire" value="entire" {{ old('sale_scope', 'entire') == 'entire' ? 'checked' : '' }} onchange="onSaleScopeChange()" style="accent-color: #3B82F6; width: 18px; height: 18px;">
                        <div>
                    ...
                    <label id="scope_plot_card" style="display: flex; align-items: center; gap: 12px; padding: 14px 18px; border-radius: 12px; border: 1.5px solid rgba(255, 255, 255, 0.15); background: rgba(16, 22, 34, 0.6); cursor: pointer; transition: all .2s ease;">
                        <input type="radio" name="sale_scope" id="sale_scope_plot" value="plot" {{ old('sale_scope') == 'plot' ? 'checked' : '' }} onchange="onSaleScopeChange()" style="accent-color: #3B82F6; width: 18px; height: 18px;">
                        <div>
            ...
            <div id="plot_select_container" class="form-group" style="display: none;">
                ...
                <div id="plot_cards_scrollbox" style="max-height: 320px; overflow-y: auto; padding: 12px; border-radius: 14px; background: rgba(10, 15, 26, 0.75); border: 1.5px solid rgba(255, 255, 255, 0.12); box-shadow: inset 0 2px 8px rgba(0,0,0,0.4);">
                    <div id="plot_cards_grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 10px;">
                        @foreach($properties as $property)
                            @php
                                $isSelected = (is_array(old('property_ids')) && in_array($property->id, old('property_ids'))) || old('property_id', request('property_id')) == $property->id;
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 14px;">
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <select name="broker_commission_type" id="sale_broker_comm_type" class="form-control" onchange="recalcSaleBrokerage()">
                            <option value="percentage" {{ old('broker_commission_type', 'percentage') === 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                            <option value="fixed" {{ old('broker_commission_type') === 'fixed' ? 'selected' : '' }}>Fixed Amount (₹)</option>
                        </select>
                    </div>
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label" style="color: #DDD6FE;">Commission Rate / Value</label>
                        <input type="number" step="0.01" name="broker_commission_rate" id="sale_broker_comm_rate" value="{{ old('broker_commission_rate') }}" class="form-control" placeholder="e.g. 2.0" oninput="recalcSaleBrokerage()">
                    </div>
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label" style="color: #A78BFA; font-weight: 700;">Total Commission (₹)</label>
                        <input type="number" step="0.01" name="broker_commission_amount" id="sale_broker_comm_amount" value="{{ old('broker_commission_amount') }}" class="form-control" placeholder="0.00" oninput="recalcSaleBrokerageDue()">
                    </div>
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label" style="color: #34D399; font-weight: 700;">Commission Paid (₹)</label>
                        <input type="number" step="0.01" name="broker_commission_paid" id="sale_broker_comm_paid" value="{{ old('broker_commission_paid', 0) }}" class="form-control" placeholder="0.00" oninput="recalcSaleBrokerageDue()">
                    </div>
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label" style="color: #FBBF24; font-weight: 700;">Commission Due (₹)</label>
                        <input type="number" step="0.01" name="broker_commission_due" id="sale_broker_comm_due" value="{{ old('broker_commission_due', 0) }}" class="form-control" placeholder="0.00" readonly style="background: rgba(0,0,0,0.15);">
                    </div>
                </div>
            </div>
        </div>
...
function onSaleScopeChange() {
    ...
    if (isEntire) {
        ...
        // Update entire property banner info
...
function updateEntirePropInfo() {
    ...
    if (selectedOpt && selectedOpt.value) {
        ...
        const valuationPrice = parseFloat(selectedOpt.dataset.price) || 0;

        titleEl.innerHTML = `🏢 Selling Entire Property: <strong>${name}</strong> ${code ? `(${code})` : ''}`;
        descEl.innerHTML = `Total Area: <strong>${area || 'N/A'}</strong> | Includes <strong>${plotsCount} Plots</strong>${purchasePrice > 0 ? ` | Purchase Cost: ₹${purchasePrice.toLocaleString('en-IN', {minimumFractionDigits: 2})}` : ''}. Enter your actual agreed sale price below.`;
    } else {
