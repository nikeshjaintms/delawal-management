...
@section('title','Add Property Document')
...
<div class="crud-header">
    <div class="crud-title">
        ...
        <p>Upload and attach a document to a property.</p>
    </div>
...
<div class="form-card">
    ...
    <div class="form-grid" style="margin-bottom:18px">
        <div class="form-group">
            ...
            <select name="property_id" id="property_id" class="form-control @error('property_id') is-invalid @enderror" required>
                ...
                @foreach($properties as $prop)
                    <option value="{{ $prop->id }}" data-project="{{ $prop->project->project_name ?? ($prop->project->propertyMaster->property_name ?? 'No Project Assigned') }}" {{ old('property_id', request('property_id')) == $prop->id ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <input type="text" name="document_title" value="{{ old('document_title') }}" class="form-control @error('document_title') is-invalid @enderror" placeholder="e.g. Sale Deed - Plot No 42" required>
            @error('document_title')<div class="text-error">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label class="form-label">Document Number</label>
            <input type="text" name="document_number" value="{{ old('document_number') }}" class="form-control @error('document_number') is-invalid @enderror" placeholder="Registration / Reference number">
    ...
    <div class="form-group" style="margin-bottom:18px">
        <label class="form-label">Upload Document <span>*</span></label>
        <input type="file" name="document_file" class="form-control @error('document_file') is-invalid @enderror" accept=".pdf,.jpg,.jpeg,.png" required>
    ...
    <div class="form-group">
        ...
        <textarea name="remarks" class="form-control @error('remarks') is-invalid @enderror" placeholder="Any additional notes about this document…">{{ old('remarks') }}</textarea>
        @error('remarks')<div class="text-error">{{ $message }}</div>@enderror
    </div>
</div>

<div class="form-action-buttons">
    <button type="submit" class="btn-primary-custom"><i class="fa fa-save"></i> Save Document</button>
