...
@section('title','Add Land Property Document')
...
<div class="crud-header">
    <div class="crud-title">
        <h2>Add Land Property Document</h2>
        <p>Upload and attach a document (7/12, NA Order, Sale Deed, etc.) to an acquired land property.</p>
    </div>
...
<div class="form-card">
    ...
    <div class="form-grid" style="margin-bottom:18px">
        ...
        <div class="form-group">
            ...
            <input type="text" name="document_title" value="{{ old('document_title') }}" class="form-control @error('document_title') is-invalid @enderror" placeholder="e.g. Sale Deed / NA Permission" required>
    ...
    <div class="form-group" style="margin-bottom:18px">
        <label class="form-label">Upload Document File <span style="font-weight:400;color:#94A3B8">(optional)</span></label>
        <input type="file" name="document_file" class="form-control @error('document_file') is-invalid @enderror" accept=".pdf,.jpg,.jpeg,.png">
        <div class="form-hint">Accepted format: PDF, JPG, JPEG, PNG — Max 5 MB</div>
        @error('document_file')<div class="text-error">{{ $message }}</div>@enderror
    </div>

    <div class="form-group">
        <label class="form-label">Remarks</label>
        <textarea name="remarks" class="form-control @error('remarks') is-invalid @enderror" placeholder="Any additional notes about this land document…">{{ old('remarks') }}</textarea>
        @error('remarks')<div class="text-error">{{ $message }}</div>@enderror
    </div>

    <div class="form-action-buttons">
        <button type="submit" class="btn-primary-custom"><i class="fa fa-save"></i> Upload Document</button>
