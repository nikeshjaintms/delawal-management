...
<div class="rpt-header">
    ...
    <div class="rpt-meta">
        <div class="rpt-title">Tenants Directory</div>
...
<div class="stat-row">
    <div class="stat-box">
        ...
        <div class="s-value">{{ $totalTenants }}</div>
    </div>
    <div class="stat-box s-green">
        <div class="s-label">Active Tenants</div>
        <div class="s-value">{{ $activeTenants }}</div>
    </div>
</div>

<div class="section-label">&#9632; Tenant Records</div>
<table>
    <thead>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($item->identity_type)
                    <span>{{ $item->identity_type }}: {{ $item->identity_number ?: '-' }}</span>
                @else
            ...
            <td>
                @if($item->emergency_contact_name)
                    <span>{{ $item->emergency_contact_name }} ({{ $item->emergency_contact_mobile ?? '-' }})</span>
                @else
                    <span style="color:#9CA3AF;">-</span>
                @endif
            </td>
        ...
        @empty
        <tr><td colspan="9" style="text-align:center;padding:20px;color:#64748B;">No tenant records found.</td></tr>
        @endforelse
    </tbody>
</table>

<div class="rpt-footer">
    <span>Delawala Management System &nbsp;—&nbsp; Tenants Directory</span>
    <span>{{ $tenants->count() }} tenants &nbsp;|&nbsp; {{ now()->format('d M Y') }}</span>
</div>
