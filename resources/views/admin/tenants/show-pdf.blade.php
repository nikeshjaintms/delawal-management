...
<div class="banner">
    <div>
        ...
        <div style="font-size:11px; color:#94A3B8;">
            Firm: {{ $tenant->firm->firm_name ?? 'Delawala Management' }} &nbsp;|&nbsp; Occupation: {{ $tenant->occupation ?: 'N/A' }}
        </div>
    </div>
    <div>
        <span class="badge-pill">STATUS: {{ strtoupper($tenant->status ?: 'ACTIVE') }}</span>
    </div>
</div>
...
<div class="grid-2">
    ...
    <div class="grid-col">
        <div class="col-heading">&#9632; Contact &amp; Identity</div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <span class="info-value">{{ $tenant->mobile ?: '-' }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Alternate Mobile:</span>
            <span class="info-value">{{ $tenant->alternate_mobile ?: '-' }}</span>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <span class="info-value">{{ $tenant->identity_type ?: 'N/A' }} {{ $tenant->identity_number ? '('.$tenant->identity_number.')' : '' }}</span>
        </div>
    </div>

    <!-- Address & Emergency -->
    <div class="grid-col">
        <div class="col-heading">&#9632; Address &amp; Emergency Contact</div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <span class="info-value">{{ $tenant->city ?: '-' }}</span>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <span class="info-value">{{ $tenant->emergency_contact_mobile ?: '-' }}</span>
        </div>
    </div>
</div>

@if($tenant->rentals && $tenant->rentals->isNotEmpty())
<div class="section-label">&#9632; Tenancy History &amp; Active Leases</div>
<table>
    <thead>
        <tr>
    ...
    <tbody>
        @foreach($tenant->rentals as $i => $r)
        <tr>
            <td style="color:#9CA3AF;">{{ $i+1 }}</td>
            <td><strong>{{ $r->property->property_name ?? '-' }}</strong></td>
            <td>{{ $r->agreement_no ?: '-' }}</td>
            <td>
                {{ $r->rent_start_date ? \Carbon\Carbon::parse($r->rent_start_date)->format('d M Y') : '-' }}
                to {{ $r->rent_end_date ? \Carbon\Carbon::parse($r->rent_end_date)->format('d M Y') : 'Ongoing' }}
            </td>
            <td class="r">₹{{ number_format($r->rent_amount, 2) }}</td>
            <td class="r">₹{{ number_format($r->security_deposit, 2) }}</td>
            <td>{{ ucfirst($r->rental_status) }}</td>
        </tr>
        @endforeach
    </tbody>
</table>
...
<div class="auth-block">
    ...
    <div class="auth-col">
        <div class="auth-line">Delawala Authorized Signatory</div>
    </div>
</div>

<div class="rpt-footer">
    <span>Delawala Management System &nbsp;—&nbsp; Tenant Dossier</span>
