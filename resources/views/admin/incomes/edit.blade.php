...
<div class="crud-header">
    <div class="crud-title">
        <h2>Edit Income</h2>
        <p>Update income record — <strong>{{ $income->income_type ?? '' }}</strong></p>
    </div>
</div>
    ...
    <form method="POST" action="{{ route('incomes.update', $income->id) }}">
                    ...
                    <select name="project_id" id="project_id" class="form-control">
                        ...
                        @foreach($projects as $proj)
                            <option value="{{ $proj->id }}"
                                    ...
                                    {{ old('project_id', $income->property?->project_id ?? '') == $proj->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="income_type" id="income_type" class="form-control @error('income_type') is-invalid @enderror" required>
                        ...
                        @foreach(['Rent','Property Sale','Commission','Interest','Other'] as $t)
                            <option value="{{ $t }}"
                                {{ old('income_type', $income->income_type) == $t ? 'selected' : '' }}>
                                {{ $t }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="payment_mode_id" id="payment_mode_id" class="form-control @error('payment_mode_id') is-invalid @enderror">
                        ...
                        @foreach($paymentModes as $mode)
                            <option value="{{ $mode->id }}"
                                {{ old('payment_mode_id', $income->payment_mode_id) == $mode->id ? 'selected' : '' }}>
                                {{ $mode->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                        <option value="active"   {{ old('status', $income->status) == 'active'   ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $income->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-gold">
                <i class="fa-solid fa-floppy-disk"></i> Update Income
            </button>
